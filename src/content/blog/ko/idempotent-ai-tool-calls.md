...
몇 년 전, 백엔드에서 같은 요청이 두 번 처리되는 것을 어떻게 막아야 하느냐는 질문을 받았습니다. 눈에 보이는 원인은 버튼을 연달아 누르는 일이었습니다. 어려운 문제는 버튼 바깥에 있었습니다.
...
예전에는 클라이언트가 멱등 키를 만드는 방식을 경계했습니다. 중요한 작업을 보호할 값을 클라이언트가 정하면 서버가 클라이언트 입력을 신뢰하는 셈이라고 생각했습니다.

지금은 이것이 서로 다른 책임을 섞은 판단이었다고 봅니다.
