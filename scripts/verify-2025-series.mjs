...
const manifestPath = new URL('scripts/2025-article-manifest.json', root);
const auditPath = new URL('scripts/2025-source-audit.json', root);
const expectedDatePolicy = 'Editorial estimates at seven-day intervals preserve ascending source ID order from 2025-01-05. They are not researched upload dates.';
...
if (!existsSync(manifestPath)) {
  failures.push('The 2025 article manifest is missing');
} else {
  try {
    manifest = JSON.parse(readFileSync(manifestPath, 'utf8'));
    if (!manifest || Array.isArray(manifest) || typeof manifest !== 'object') {
      failures.push('The 2025 article manifest must be an object, not a bare article array');
      manifest = {};
    }
    if (!Array.isArray(manifest.articles)) failures.push('The 2025 article manifest must contain an articles array');
    entries = Array.isArray(manifest.articles) ? manifest.articles : [];
  } catch (error) {
    failures.push(`The 2025 article manifest is invalid JSON: ${error.message}`);
    manifest = {};
  }
}
...
if (manifest) {
  if (!exactKeys(manifest, expectedManifestKeys)) failures.push('The 2025 manifest must use the exact approved top-level schema');
  if (manifest.sourceYear !== 2025) failures.push('The manifest source year must be 2025');
  if (manifest.sourceCount !== 51) failures.push('The complete 2025 corpus must contain 51 logical Markdown sources');
  if (manifest.retainedCount !== 50) failures.push('The manifest must retain exactly 50 publishable sources');
  ...
  if (entries.length + excludedSources.length !== manifest.sourceCount) failures.push('Every 2025 source must be retained or explicitly excluded');
  ...
  const expectedSourceIds = [
    ...
    '36', '37', '38', '41', '42', '43', '44', '45', '47', '48', '49', '50', '52', '57', '59', '60', '61',
  ];
  if (new Set(sourceFiles).size !== manifest.sourceCount) failures.push('Every 2025 source file must appear exactly once in the manifest');
  if (new Set(sourceIds).size !== expectedSourceIds.length || expectedSourceIds.some((sourceId) => !sourceIds.includes(sourceId))) {
    failures.push('The manifest must contain the exact 51 logical 2025 source IDs once each');
  }
  if (allSources.some((entry) => !String(entry.sourceFile ?? '').normalize('NFC').startsWith(`25_${String(entry.sourceId).padStart(2, '0')}_`))) {
  ...
  if (technicalCount !== 32 || developerCount !== 18) failures.push('The retained corpus must contain 32 technical and 18 developer-practice sources');
  if (
    excludedSources.length !== 1 ||
    String(excludedSources[0]?.sourceId) !== '19' ||
    excludedSources[0]?.classification !== 'exclude-self-promotion'
  ) {
    failures.push('Only source 19 may be excluded, for its self-promotional service-launch and recruitment focus');
  }
...
try {
  ...
  failures.push(`The 2025 source audit is missing or invalid: ${error.message}`);
}
if (!exactKeys(audit, expectedAuditKeys)) failures.push('The 2025 source audit must use the exact approved top-level schema');
if (audit.sourceYear !== 2025 || audit.sourceCount !== 51 || audit.datePolicy !== expectedDatePolicy) failures.push('The 2025 source audit metadata is invalid');
const auditedSources = Array.isArray(audit.sources) ? audit.sources : [];
if (auditedSources.length !== 51) failures.push('The 2025 source audit must contain exactly 51 sources');
...
if (new Set(auditedSources.map((source) => String(source?.sourceId))).size !== 51) failures.push('Every audited source ID must appear exactly once');
...
for (const [index, entry] of entries.entries()) {
  ...
  if (!/^2025-\d{2}-\d{2}$/u.test(expectedDate)) failures.push(`Invalid 2025 date for ${slug}: ${expectedDate}`);
  ...
  const combined = `${en}\n${ko}`;
  if (/\b20(?:2[6-9]|[3-9]\d)\b/u.test(`${enBody}\n${koBody}`)) failures.push(`Post-2025 facts appear in the source-era body for ${slug}`);
...
if (new Set(slugs).size !== slugs.length) failures.push('The 2025 manifest contains duplicate slugs');
if (new Set(dates).size !== dates.length) failures.push('Every retained 2025 source needs a unique publication date');
if (dates[0] !== '2025-01-05' || dates.at(-1) !== '2025-12-14') failures.push('The approved estimated date range must remain 2025-01-05 through 2025-12-14');
const sourceOrders = entries.map((entry) => entry.order);
if (new Set(sourceOrders).size !== sourceOrders.length) failures.push('Retained 2025 sources need unique source-order positions');
for (let index = 1; index < dates.length; index += 1) {
  if (sourceOrders[index - 1] >= sourceOrders[index]) failures.push(`2025 source order must increase at manifest index ${index}`);
  if (dates[index - 1] >= dates[index]) failures.push(`2025 publication dates must increase in source order at manifest index ${index}`);
  const previous = new Date(`${dates[index - 1]}T00:00:00Z`);
  const current = new Date(`${dates[index]}T00:00:00Z`);
  if (current - previous !== 7 * 24 * 60 * 60 * 1000) failures.push(`2025 estimated dates must stay exactly seven days apart at manifest index ${index}`);
}

function published2025Slugs(locale) {
  const directory = new URL(`src/content/blog/${locale}/`, root);
  return readdirSync(directory)
    .filter((name) => name.endsWith('.md'))
    .filter((name) => /^2025-/u.test(value(readFileSync(new URL(name, directory), 'utf8'), 'publishedAt')))
    .map((name) => name.slice(0, -3))
    .sort();
}

const expectedSorted = [...slugs].sort();
if (JSON.stringify(published2025Slugs('en')) !== JSON.stringify(expectedSorted)) failures.push('English 2025 sources must match the manifest exactly');
if (JSON.stringify(published2025Slugs('ko')) !== JSON.stringify(expectedSorted)) failures.push('Korean 2025 sources must match the manifest exactly');

if (failures.length > 0) {
  console.error(`2025 article contract failed (${failures.length}):`);
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`2025 article contract passed (${entries.length} bilingual pairs)`);
